...
import { getProducts, puchaseAdd, purchaseUpdate } from "../../Redux/clientSlice/clientSlice";
...
const PurchaseForm = () => {
    ...
    const create_product_handler = async (formType) => {
        if (location.state.action === "update" && location.state.type === "purchase") {
            dispatch(purchaseUpdate({ data: clientFormData.productsPurchased, id: location.state?.clientId, type: formType, state: location.state }))
        }
        if (location.state.action === "add" && location.state.type === "purchase") {
            dispatch(puchaseAdd({ data: clientFormData.productsPurchased, id: location.state?.clientId, type: formType, state: location.state }))
        }
    }
    ...
    const formInputHandler = (e) => {
        ...
        setClientFormData({
            ...clientFormData,
            productsPurchased : {
                ...clientFormData.productsPurchased,
    ...
    useEffect(() => {
        dispatch(getProducts({type_ : "product"}))
        ...
        if (location.state && location.state.type === "purchase" && location.state.action === "update") {
            setClientFormData({
                ...clientFormData,
                productsPurchased: location.state.value
            })
        }
                ...
                <div className="flex flex-col md:flex-row md:gap-x-3 justify-center items-center relative bg-white w-[90vw] min-h-[102vh] md:min-h-[76vh] left-[5vw] top-[14vh] rounded-md shadow-md py-5">
                    ...
                    <div className="order-2 md:order-1 h-max min-h-[55vh] md:min-h-[88%] md:w-[65%] w-[78vw] bg-transparent mx-auto md:mx-0 z-10 grid grid-cols-12" style={{ placeContent: "flex-start" }}>
                        ...
                        />
                            ...
                            <OptionSelect fieldsetClassName={fieldsetClassName} labelHtmlFor="productName" labelClassName={labelClassName} labelText="Product Name" inputDefaultValue="" id="productName" inputName="productName" inputClassName={inputClassName} selectOptionFun={formInputHandler} values ={products || []} />
                            <Input inputOnchangeFun={formInputHandler} inputName="amount" inputType="number" inputDefaultValue={clientFormData.productsPurchased.amount} labelHtmlFor="Amount" labelText="Amount" fieldsetClassName={fieldsetClassName} labelClassName={labelClassName} inputClassName={inputClassName} />
                            <fieldset className="col-span-12 lg:col-span-6 flex flex-wrap gap-x-2 items-center px-2 py-1 my-1">
                                ...
                                <button onClick={() => {
                                    ...
                                    setClientFormData({
                                        ...
                                        productsPurchased: {
                                            ...clientFormData.productsPurchased,
                                            other: [...clientFormData.productsPurchased.other, newString ]
                                        }
                                    });
                                ...
                                <span className="w-full flex flex-wrap gap-x-1.5 gap-y-1 mt-2">
                                    {clientFormData.productsPurchased?.other?.map((v, i) => {
                                        return <span
                                            onClick={() => {
                                                document.getElementById("other").value = v;
                                                setClientFormData({ ...clientFormData, productsPurchased: { ...clientFormData.productsPurchased, other: clientFormData.productsPurchased.other.filter((f, fi) => f !== v) } })
                                            }
                                            }
