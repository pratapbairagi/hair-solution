...
import Th from "./th"
import { deleteTakenService, getClient, reviewMail } from "../../Redux/clientSlice/clientSlice";
...
const ServiceTable = () => {
    const { client, loading, success, error, message, type } = useSelector(state => state.client)
    ...
    const deleteDataHandler = ({ _id }) => {
        dispatch(deleteTakenService({ _id, id: client._id }))
    }

    const selectDataToUpdate = ({ value, type, action }) => {
        navigate("/service/update", { state: { value: value, type: type, action: action, clientId: client._id } })
    }
    ...
    const array = useMemo(() => {
        if (client) {
            return client.servicesTaken?.map((v, i) => {
                return <tr key={i} className="bg-white lg:hover:bg-gray-100 flex lg:table-row flex-row lg:flex-row flex-wrap lg:flex-no-wrap mb-10 lg:mb-0">
                    <Td header="Service Name" extraClass="max-w-[68%] lg:max-w-[100%]" value={v.serviceName} />
                    <Td header="Service Amount" value={v.amount} />
                    <Td header="Service Date"
                        value={moment(v.serviceDate).tz("Asia/Kolkata").format('YYYY-MM-DD HH:mm:ss')} />
                    ...
                    <td className="w-full flex gap-x-2 justify-end items-center flex-row lg:w-auto p-3 text-gray-800 text-end lg:text-center border border-b block lg:table-cell relative lg:static lg:max-w-[260px] ">
                        ...
                        <div className="flex lg:flex-wrap lg:gap-y-1 gap-x-1 justify-center">
                        <button onClick={() => selectDataToUpdate({ value: v, type: "service", action: "update" })} className="bg-blue-400 text-sm font-semibold text-gray-100 px-3 py-1 rounded-sm hover:bg-gray-100 hover:text-blue-400">Update</button>
                        ...
                        <button onClick={() => reviewMail_fun({entityId : v._id, type :"servicesTaken" })} className="bg-green-400 lg:ml-3 text-sm font-semibold text-gray-100 px-3 py-1 rounded-sm hover:bg-green-500">Review Mail</button>
                        </div>
                    </td>
                </tr>
            });
    ...
    return (
        <>
            <div className="w-[100vw] flex flex-col md:justify-start items-start p-1 lg:p-4 min-h-[100vh] pt-[16vh] lg:pt-[18vh] bg-white" style={{ maxWidth: "100%", overflow: "auto" }}>
            <ToastContainer 
            ...
            autoClose="4000"
             />
                ...
                <h5 className="font-bold text-gray-700 text-start px-1.5 lg:px-4 text-xl flex justify-start w-full mb-5">
                    Service
                    <button onClick={() => selectDataToUpdate({ value: null, type: "service", action: "add" })} className="bg-orange-600 h-max text-white text-sm px-2.5 py-0.5 lg:px-4 py-1 rounded-sm hover:bg-orange-500 ml-auto">Add</button>
                    <button onClick={() => {
                        navigate(-1)
